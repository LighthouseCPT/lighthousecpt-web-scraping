...
class PDFTables(object):
    ...
    def convert(self, pdf_file_or_path, out_path=None, out_format=None, query_params=None, **requests_params):
        """
        Convert PDF given by `pdf_file_or_path` into `format` at `out_path`.

        If `out_path` is None, returns a string containing the contents, or a
        bytes for binary output types (e.g, XLSX)
        """
        (out_path, out_format) = PDFTables.ensure_format_ext(out_path, out_format)

        # If pdf_file_or_path is a string, treat it as file path;
        # If pdf_file_or_path is a bytes object or file object, pass directly to the request.
        if isinstance(pdf_file_or_path, str):
            with open(pdf_file_or_path, 'rb') as pdf_fo:
                response = self.request(pdf_fo, out_format, query_params, **requests_params)
        else:
            response = self.request(pdf_file_or_path, out_format, query_params, **requests_params)
    ...
    def request(self, pdf_fo, out_format=None, query_params=None,
        ...
        (_, out_format) = PDFTables.ensure_format_ext(None, out_format)
        url = self.api_url
